...
| 단어                                       | 내용                                                         | 기타 |
| ------------------------------------------ | ------------------------------------------------------------ | ---- |
| SW개발 보안 3대 요소                       | 기밀성, 무결성, 가용성                                       |      |
| 기밀성                                     | - 인가되지 않은 접근에 정보 노출을 차단하는 특성             |      |
| 무결성                                     | - 정당한 방법으로만 데이터를 변경할 수 있으며, 데이터의 정확성을 보장하는 특성 |      |
| 가용성                                     | - 권한을 가지고 있으며 서비스를 지속해서 사용할 수 있도록 하는 특성 |      |
| DoS 공격                                   | - 시스템의 자원을 고갈시켜 서비스 거부를 유발하는 공격       |      |
| SYN 플러딩<br />SYN Flooding               | - ACK를 발송하지 않고 SYN 패킷만 보내 점유하여 자원을 고갈시키는 공격 |      |
| UDP 플러딩<br />UDP Flooding               | - 대량의 UDP 패키지를 임의의 포트번호로 전송하지만, 응답 메세지는 공격자에 전달되지 않아 자원을 고갈시키는 공격 |      |
| 스머프/스머핑<br />smurf/smurfing          | - 출발지 IP를 공격 대상의  IP로 변조하여, 브로드캐스팅으로 ICMP Echo 패킷 요청을 보내 과부하시키는 공격 |      |
| 죽음의 핑<br />Ping of Death               | - ICMP 패킹(핑)을 아주 크게 만들어 과부하시키는 공격         |      |
| 랜드 어택<br />Land Attack                 | - 출발지 IP와 도착지 IP를 같은 주소로 만들어, 자기 자신에게 응답을 보노도록 하는 공격 |      |
| 티어 드롭<br />Tear Drop                   | - 조작된 IP 패킷 조각을 보내 재조립 과정에서 오류를 발생시키는 공격 |      |
| 봉크/보잉크<br />Bonk/Boink                | - IP 패킷의 재전송, 재조합 과정에서 오류를 발생시키는 공격   |      |
| DDos 공격                                  | - 여러 대의 공격자를 분산 배치 후 동시에 동작시키는 DoS 공격 |      |
| 스니핑 (Sniffing)                          | - 공격대상에 직접 공격을 가하지 않고, 몰래 정보를 들여다보는 수동적인 공격 기법 |      |
| 트로이목마                                 | - 겉으로는 정상적이나 실행하면 악성코드가 실행되는 프로그램  |      |
| 버퍼 오버플로우 공격 대응                  | - 스택가드<br />- 스택쉴드<br />- ASLR<br />- 안전한 함수 사용<br />- 실행 제한 |      |
...
| 암호화 알고리즘 방식                       | 1. 대칭 키 암호 방식<br />2. 비대칭 키 암호 방식<br />3. 해시 암호 방식 |      |
| 대칭 키 암호화 종류                        | - 블록 암호 (DES,3DES,ARS,SEED,ARIA,IDEA)<br />- 스트림 암호 (LFSR, RC4) |      |
| 비댕칭 키 암호화 종류                      | - 디피-헬만<br />- RSA<br />- ECC<br />- Elgamal             |      |
| 블록 암호                                  | - 고정 길이의 블록을 암호화하는 대칭 키 암호 방식            |      |
| 스트림 암호                                | - 매우 긴 주기의 난수열을 발생시켜 암호화하는 대칭 키 암호 방식 |      |
| 일방향 암호                                | - 임의의 길이의 정보를 입력받아 고정된 길이의 암호문을 출력하는 암호 방식 |      |
| MAC                                        | - Message Authenication Code<br />- 키를 사용하는 메시지 인증 코드로 무결성을 보장하는 암호 알고리즘 |      |
| MDC                                        | - Modification Detection Code<br />- 키를 사용하지 않는 변경 감지 코드로 무결성을 보장하는 암호 알고리즘 |      |
| DES                                        | - 대칭키<br />- 미 연방표준국(NIST)에서 발표<br />- 키 길이는 56bit, 블록크기는 64bit |      |
| AES                                        | - 대칭키<br />- Advanced Encryption Standard.<br />- 3DES의 문제점을 극보하기 위해 개발<br />- 라운드 수는 10,12,14 분류되고, 한 라운드는 빼고 이동하고 섞고 더하는 4계층으로 구성됨 |      |
| SEED                                       | - 대칭키<br />- KISA가 개발 16라운드를 거쳐 128비트 블록으로 암호화 |      |
| ARIA                                       | - 대칭키<br />- Academy + Research Institute + Agency<br />- 국정원과 산학연구협회가 개발함 |      |
| IDEA                                       | - 대칭키<br />- DES를 개체하기 위해 개발                     |      |
| LFSR                                       | - 대칭키<br />- 선형 되먹임 시프트 레지스터                  |      |
| RC4                                        | - 대칭키<br />- 셔플링 기법을 이용해 평문과 XOR 연산해 암호화 |      |
| 디피-헬만                                  | - 비대칭키<br />- 최초의 비밀키 교환 프로토콜<br />- 이산대수 계산의 어려움을 근거로 함 |      |
| RSA                                        | - 비대칭키<br />- 수학교수 3명의 앞글자를 땃음<br />- 소인수 분해 문제의 어려움을 근거로 함 |      |
| ECC                                        | - 비대칭키<br />- 타원 곡선 암호<br />- RSA의 대안           |      |
| ElGamal                                    | - 비대칭키<br />- 이산대수의 어려움을 근거로 함<br />- 전자서명에 사용 가능 |      |
| MD5                                        | - 해시암호화<br />- MD4를 개선한 알고리즘                    |      |
| SHA-1                                      | - 해시 암호화<br />- NAS에서 미 정부 표준으로 지정<br />- DSA에서 사용 |      |
| SHA-256/384/512                            | - 해시 암호화<br />- 256비트의 해시값을 생성하는 함수        |      |
| HAS-160                                    | - 해시 암호화<br />- 국내 표준 디지털 서명 알고리즘(KCDSA)를 위해 개발된 알고리즘<br />- MD5의 장점 + SHA-1 의 장점 |      |
| IPSec                                      | - 네트워크 계층(3계층)에서 사용하는 보안 프로토콜<br />- AH(인증)프로토콜: MAC를 통해 인증 제공<br />- ESP(암호화) 프로토콜 : MAC+암호화를 통해 인증+기밀성 제공<br />- IKE(키관리) 프로토콜 : Key를 주고받는 알고리즘 |      |
| SSL/TLS                                    | - 전송계층(4계층)과 응용계층(7계층) 사이에서 안전한 데이터 전송을 보장하는 보안 프로토콜 |      |
| S-HTTP                                     | - 클라/서버 간 메시지를 암호화하는 보안 기술                 |      |
...
| CSRF                                       | - Cross Site Request Forgery (사이트간 요청 위조) <br />- 사용자가 자신의 의지와 무관하게 공격자가 의도한 행위를 요청하게 하는 공격 |      |
| SQL Indection                              | - 악의적인 SQL 구문을 삽입, 실행시켜 DB 정보를 탈취하거나 조작하는 공격 |      |
| Form SQL Injection                         | - HTML Form 기반 인증의 취약점을 이용한 공격                 |      |
| Union SQL Injection                        | - UNION 연산자를 이용해 쿼리 결과를 결합해 공격              |      |
| FireWall                                   | - 내부,외부 트래픽을 모니터링하여 접근을 허용/차단하는 시스템 |      |
| WAF                                        | - 웹 애플리케이션에 특화되어, XSS나 SQL Injection 등을 탐지하고 차단하는 시스템 |      |
| 네트워크 접근 제어 (NAC)                   | - 내부 네트워크에 접속을 시도할 때 통제하는 솔루션           |      |
| 침임 탐지 시스템 (IDS)                     | - 비인가 사용자의 침입을 실시간으로 탐지하는 시스템          |      |
| 침입 방지 시스템 (IPS)                     | - 공격 및 침입을 실시간으로 차단하는 시스템                  |      |
| 가상 사설망 (VPN)                          | - 공중망을 사용할 때 마치 전용망을 사용하는 것과 같은 보안 효과를 주는 솔류션 |      |
| Secure OS                                  | - OS 커널에 보안 기능을 추가한 솔류션                        |      |
| Anti-Spam Solution                         | - 메일 서버 앞단에 바이러스 검사, 정보 유출 방지 등의 기능을 제공하는 솔루션 |      |
| DLP                                        | - Data Loss Prevention<br />- 주요 자료가 외부로 유출되는 것을 차단하는 솔류션 |      |
| DRM                                        | - Digital Right Mangemnet<br />- 디지털 저작물에 암호를 걸어, 권한이 없는 사용자의 사용을 막는 솔루션 |      |
| BCP                                        | - 비즈니스 연속성 계획 (Business Continuity Plna)<br />- 위기관리를 기반으로 비상 시 비즈니스 연속성을 보장하는 체계<br />- BIA가 선행되어야 함 |      |
...
| RPO                                        | - 복구 지점 목표 (Recovery Point Object)<br />- 재해 시 업무중단 시점 ~ 정상기동까지 허용하는 손실 |      |
| DRP                                        | - 재해 복구 계획 (Disaster Recovery Plan)<br />- 재해로 장기간 운영이 불가한 경우를 대비한 계획 |      |
| DRS                                        | - 재해 복구 시스템 (Disaster Recovery System)<br />- DRP를 위한 관리체계 |      |
| DRS의 유형                                 | - Mirror Site<br />- Hot Site<br />- Warm Site<br />- Cold Site |      |
| Mirror Site                                | - 주 센터 & 복구센터 모두 운영 상태 (RTO는 0)                |      |
| Hot Site                                   | - 주 센터와 동일한 수준의 자원을 대기 상태로 보유하며 데이터를 최신 상태로 유지 (RTO는 4시간 이내) |      |
...
| 원드토커 (WindTalker)                      | - 터치, 타이핑 등의 패턴을 스니핑하여 해킹                   |      |
| DNL 스푸핑                                 | - DNS 서버 캐시를 조작해 의도치 않은 주소로 접속하게 하는 공격 |      |
| 포트 스캐닝 (Port Scanning)                | - 침입 전 어떤 포트가 활성화 되어 있는 확인하는 기법         |      |
| 디렉토링 리스팅 취약점 (Directory Listing) | - 웹 서버의 인덱싱 기능이 활성화된 경우, 서버 내 모든 디렉토리를 볼 수 있는 취약점 |      |
| 멜트다운 (Metldown)                        | - 인텔 아키텍처의 버그를 이용해 시스템 메모리에 접근하는 취약점 |      |
| 허니팟 (HoneyPot)                          | - 일부러 허술하게 만들어 해커에게 노출하는 유인시스템        |      |
| 핑거프린트 (Finger Printing)               | - 저작권 정보와 구매자 정보를 콘텐츠에 삽입해 불법 배포자를 추적할 수 있는 기술 |      |
